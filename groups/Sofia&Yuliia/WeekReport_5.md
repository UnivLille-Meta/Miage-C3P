...
Double Dispatch means that a program chooses the right method depending on both objects — the one that sends the message and the one that receives it. This technique lets two objects decide together what to do. 
For example, in the Stone–Paper–Scissors game, each object knows how to play against another:
